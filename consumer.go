...
func emailWorker(
	...
	for recipient := range ch {
		...
		if err != nil {
			...
			if err := postgres.RecordEmailEvent(
				recipient.CampaignID,
				recipient.Email,
				"failed",
				recipient.Retry,
			); err != nil {
				log.Printf("postgres failed event error: %v\n", err)
			}
			...
			if willRetry {
				if err := postgres.RecordEmailEvent(
					...
					log.Printf("postgres retry event error: %v\n", err)
				}
				...
				appendActivityLog(
					...
					recipient.Email,
					fmt.Sprintf(
						"worker %d: send failed, queued for retry (%d/%d)",
						id,
						recipient.Retry+1,
						MAX_RETRIES,
					),
				...
				if err := postgres.RecordEmailEvent(
					...
					log.Printf("postgres dlq event error: %v\n", err)
				}
				...
				appendActivityLog(
					...
					recipient.Email,
					fmt.Sprintf(
						"worker %d: send failed, moved to DLQ after %d attempts",
						id,
						recipient.Retry+1,
					),
				)
		...
		if err := postgres.RecordEmailEvent(
			...
			log.Printf("postgres sent event error: %v\n", err)
		}

		appendActivityLog(
			"sent",
			recipient.Email,
			fmt.Sprintf("worker %d: email sent successfully", id),
		)

		fmt.Printf(
			"Worker %d: email sent successfully to %s\n",
			id,
			recipient.Email,
		)
	}
...
// pushToRetryQueue handles retry logic:
// increment retry count, then push to retry queue or DLQ.
func pushToRetryQueue(client *redislib.Client, recipient Recipient) {
	...
	if recipient.Retry > MAX_RETRIES {
		...
		log.Printf(
			"Email %s moved to DLQ after %d retries\n",
			recipient.Email,
			recipient.Retry-1,
		)
	...
	log.Printf(
		"Email %s queued for retry (Attempt %d/%d)\n",
		recipient.Email,
		recipient.Retry,
		MAX_RETRIES,
	)
